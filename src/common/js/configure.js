/*
 * pwix:accounts-core/src/common/js/config.js
 */
...
let _conf = {};
AccountsCore._conf = new ReactiveVar( _conf );
 ...
 */
AccountsCore.configure = function( o ){
    if( o && _.isObject( o )){
        // check that keys exist
        let built_conf = {};
        Object.keys( o ).forEach(( it ) => {
            if( Object.keys( AccountsCore._defaults ).includes( it )){
                built_conf[it] = o[it];
            } else {
        ...
        if( Object.keys( built_conf ).length ){
            _conf = _.merge( AccountsCore._defaults, _conf, built_conf );
            AccountsCore._conf.set( _conf );
            logger.verbose({ verbosity: _conf.verbosity, against: AccountsCore.C.Verbose.CONFIGURE }, 'configure() with', built_conf );
        }
    }
    // also acts as a getter
    return AccountsCore._conf.get();
}

_conf = _.merge( {}, AccountsCore._defaults );
AccountsCore._conf.set( _conf );
...
// as soon as we have an Accounts manager, we need amAccount'es, and so must not leave this AccountsCore creates itself the 'users' collection
Tracker.autorun(() => {
    const autoUsers = AccountsCore.configure().autoUsers;
